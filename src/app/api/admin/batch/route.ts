  ...
  const [batches] = await db.query(
    `SELECT b.*,
      (SELECT JSON_ARRAYAGG(JSON_OBJECT(
        ...
        'bake_date', DATE_FORMAT(bi.bake_date, '%Y-%m-%d'),
     ...
     FROM batches b ORDER BY b.created_at DESC LIMIT 20`
  ) as any[]
...
export async function POST(req: NextRequest) {
  ...
  if (!pickup_window) {
    return NextResponse.json({ error: 'Pickup window required' }, { status: 400 })
  }

  const batchId = randomUUID()
  await db.query(
    `INSERT INTO batches (id, pickup_window, notes, status) VALUES (?, ?, ?, 'draft')`,
    [batchId, pickup_window, notes || null]
  )

  for (const item of inventory || []) {
    if (item.qty > 0 && item.bake_date) {
      await db.query(
...
export async function PATCH(req: NextRequest) {
  ...
  if (action === 'open') {
    ...
    const emails = (waitlist as any[]).map((w: any) => w.email)
    if (emails.length > 0) {
